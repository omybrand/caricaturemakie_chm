...
function mainController($scope, $http) {
	$scope.formData = { yy: '', mm: '', artist_sn: '' };
	$scope.sum = 0;
	$scope.cnt  = 0;
	$scope.mileage = 0;
	...
	$http.get('/chm/artist/getdate').success(function (data) {
		...
		$scope.formData.artist_sn = document.getElementById("artist_sn").value;
		...
		$http.post('/chm/manage/carical/listap', $scope.formData).success(function (datas) {
			for (var i in datas) {
				...
				$scope.sum1 += datas[i].price;
				$scope.mileage += datas[i].mileage;
			}

			$scope.cnt = datas.length;
			$scope.cnt1 = datas.length;
			$scope.todos = datas;
			...
			$http.post('/chm/manage/carical/listom', $scope.formData).success(function (datas2) {
				for (var i in datas2) {
					...
					$scope.sum2 += datas2[i].price;
				}
				$scope.cnt += datas2.length;
				$scope.todos2 = datas2;
				$scope.cnt2 = datas2.length;
				...
				$http.post('/chm/manage/carical/listco', $scope.formData).success(function (datas3) {
					for (var i in datas3) {
						...
						$scope.sum3 += datas3[i].price;
					}
					$scope.cnt += datas3.length;
					$scope.cnt3 = datas3.length;
					$scope.todos3 = datas3;
				}).error(function (err) {
